...
fun main() {
    ...
    do {
        ...
        when (choice) {
            ...
            5 ->{
                val index = readInt("Enter the index to insert at: ")
                array = insertSubArray(array, index)
            }
...
fun insertSubArray(array: Array<Int>,index: Int): Array<Int>{
    var lengthOfSubarray = readInt("Length of Subarray: ")
    var subArray = Array(lengthOfSubarray){0};
    for(i in 0..lengthOfSubarray-1){
         var value = readInt("Enter the value for $i: ")
         subArray[i] = value
    }
    ...
    val newArray = Array(array.size + lengthOfSubarray) { 0 }
    for (i in 0..<index) { // for(i in 0..<index){
        newArray[i] = array[i]
    }
    for (i in 0..<lengthOfSubarray){
        newArray[index+i] = subArray[i]
    }

    for (i in index..<array.size) {
        newArray[i+lengthOfSubarray] = array[i]
    }
    return newArray
}
